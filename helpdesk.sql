...
INSERT INTO `categorias` VALUES (1,'Computadoras e Impresoras','Problemas relacionados con hardware físico, como fallos en encendido, lentitud en equipos o errores en impresión (ej. atascos de papel o conexión de impresoras).',1),(2,'Sistema y Programas','Incidencias en el software del sistema operativo o aplicaciones específicas, como errores al abrir programas, actualizaciones fallidas o bloqueos inesperados.',1),(3,'Redes e Internet','Dificultades con la conectividad, como falta de acceso a internet, problemas con Wi-Fi, redes internas o conexiones remotas lentas o inestables.',1),(4,'Cuentas y Contraseñas','Cuestiones de acceso a cuentas, como olvido de contraseñas, bloqueos de usuario, creación de nuevas cuentas o problemas de autenticación.',1),(5,'Reloj Checador','Errores en el sistema de registro de asistencia, como fallos en el reloj biométrico, problemas de sincronización o discrepancias en los registros de entrada/salida.',1),(6,'Seguridad y Prevención','Amenazas o alertas de seguridad, como virus, accesos no autorizados, configuraciones de firewall o medidas preventivas para proteger datos y equipos.',1),(7,'Otros','Consultas generales o problemas no clasificados',1);
...
CREATE TABLE `tickets` (
  ...
  `t_close_user` int DEFAULT NULL,
  `t_equip` varchar(100) COLLATE utf8mb4_spanish_ci DEFAULT NULL,
  PRIMARY KEY (`t_id`),
  ...
  KEY `fk_tickets_close_user` (`t_close_user`),
  ...
  CONSTRAINT `fk_tickets_close_user` FOREIGN KEY (`t_close_user`) REFERENCES `empleados` (`e_id`)
...
-- Dump completed on 2025-11-20  3:40:19
